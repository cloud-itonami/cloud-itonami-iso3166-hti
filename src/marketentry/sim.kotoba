...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (HTI, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up marche-public-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (marche-public-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up blacklist-unverified) ==")
    ...
    (println "== filing/submit eng-5 (blacklist-unverified -> HARD hold) ==")
