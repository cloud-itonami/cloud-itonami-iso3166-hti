(ns marketentry.marketentryllm
  ...
  the Republic of Haiti (HTI) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real CNMP filing / MCI
  Autorisation de Fonctionnement registration. Every output is censored
  downstream by `marketentry.governor` before anything touches the SSoT,
  and `:filing/draft`/`:filing/submit` proposals NEVER auto-commit at any
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world CNMP filing / MCI
  Autorisation de Fonctionnement registration request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けCNMP/MCI提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "contracting-authority-category=" (:contracting-authority-category e)
                        " contract-value-htg=" (:contract-value-htg e)
                        " claimed-marche-public?=" (:claimed-marche-public? e)
                        " blacklist-clear?=" (:blacklist-clear? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-blacklist-check? e))
                              (:blacklist-clear? e)))
                   0.9 0.3)}))
