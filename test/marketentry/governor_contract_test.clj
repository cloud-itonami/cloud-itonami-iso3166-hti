  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Société Anonyme Peyi Byen Ranje"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Société Anonyme Peyi Byen Ranje" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest marche-public-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-marche-public? does not match the independently recomputed Arrêté du 23 février 2026 Art.3/4/5/6 seuil de passation -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:marche-public-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest blacklist-unverified-is-held-and-unoverridable
  (testing "unverified CNMP Liste Noire check when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:blacklist-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
